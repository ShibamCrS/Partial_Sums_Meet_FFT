...
int main(void) {
    srand(4057218);
    for (int log_n = 32; log_n <= 32; ++log_n) {
        printf("%d ", log_n);
        size_t n = 0x1LL << log_n;
        size_t size_size = n*sizeof(float) + 32;
        void *buf = malloc(size_size);
        char *start = buf;
        while ((size_t)start % 32 != 0) start = start + 1;
        float *a = (float*)start;
        size_t size_size_1 = n * sizeof(float);
        float *aux = (float*)malloc( size_size_1);
        for (int i = 0; i < n; ++i) {
            a[i] = 1.0 - 2.0 * (rand() & 1);
            aux[i] = a[i];
        }
