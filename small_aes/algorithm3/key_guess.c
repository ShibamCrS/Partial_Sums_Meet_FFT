...
/************************************************************************
 *All the circuit_evaluation function and fht_step functions are in FHT_computations.c
 ************************************************************************/
void* key_guess_0(void *args1){
    Step2Args *args = (Step2Args*) args1;
    uint64_t IS_B[1ULL << (WORD*2)];
    circuit_evaluation_2(IS_B);
    uint64_t IS_C[1ULL << (WORD*2)];
                ...
                for(uint64_t k4=0; k4<(1UL<<WORD); k4++){
                    ...
                    if(bt_get(args->RemovedKeys, pos) == 0){
                        ...
                        for(uint64_t c3c2a=0UL; c3c2a<(1UL << (WORD*3)); c3c2a++){
                            uint64_t k1k0_index = k1k0 - (args->start);
                            uint64_t pos1 = (k1k0*(1UL<<(WORD*3)))+c3c2a;
                            if(bt_get(args->A, pos1) == 1){
                                uint64_t c3 = (c3c2a >> (2*WORD)) & 0x0F;
                                uint64_t c2 = (c3c2a >> WORD) & 0x0F;
                                uint64_t a = c3c2a & 0x0F;
...
uint64_t filter_keys(bitmap *A, bitmap *RemovedKeys, uint64_t data_set_index){
    ...
    uint64_t len_a_each_thread = len_a / THREADS;
    uint64_t len_a_each_thread_arr_size = len_a_each_thread >> 6;
